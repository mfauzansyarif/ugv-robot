...
# Create a PanedWindow to allow resizing the left and right sides
paned_window = ttk.Panedwindow(root, orient=tk.HORIZONTAL)
paned_window.pack(fill=tk.BOTH, expand=True)

# Left frame for Serial Communication
frame_ports = ttk.Frame(paned_window)
paned_window.add(frame_ports, weight=1)
...
# Start/Stop Button for Transmit Data
start_stop_frame = ttk.Frame(frame_ports)
start_stop_frame.pack(pady=5)

start_button = ttk.Button(start_stop_frame, text="Start", command=open_serial_ports)
...
frame_video = ttk.Frame(paned_window)
paned_window.add(frame_video, weight=2)
