...
def relay_data(serial_in, serial_out, terminal_text, data_display):
    """
    ...
    while True:
        try:
            if serial_in.in_waiting > 0:
                ...
                # Display data received in the "Data Received to Device" box
                #data_display.insert(tk.END, f"Sent: {decoded_data}\n")
...
def open_serial_ports():
    ...
    try:
        ...
        ser1 = serial.Serial(port1_var.get(), baudrate, timeout=1, rtscts=False)  # Receiving port
        ser2 = serial.Serial(port2_var.get(), baudrate, timeout=1, rtscts=True)  # Sending port

        terminal_text.insert(tk.END, f"Opened ports: {port1_var.get()} (receive) and {port2_var.get()} (send)\n")
        terminal_text.yview(tk.END)
        ...
        # Start threads to relay data and read from the receiving port
        threading.Thread(target=relay_data, args=(ser1, ser2, terminal_text, data_display), daemon=True).start()
...
# Set window size to 80% width and height of the screen
window_width = root.winfo_screenwidth() * 0.95
window_height = root.winfo_screenheight() * 0.95
root.geometry(f"{int(window_width)}x{int(window_height)}")

# Frame for Serial Communication on the left
frame_ports = ttk.Frame(root)
frame_ports.pack(side=tk.LEFT, padx=10, pady=10, fill=tk.Y)

# COM Port selection for Send
frame_send = ttk.Frame(frame_ports)
frame_send.pack(pady=5)

ttk.Label(frame_send, text="COM CH340(Receive):").grid(row=0, column=0, padx=5, pady=5)
...
ttk.Label(frame_receive, text="COM CP2102(Send):").grid(row=0, column=0, padx=5, pady=5)
...
# Create the video feed and adjust its size to occupy 4/5 of the screen
class MyVideoCapture:
    def __init__(self, video_source=0) -> None:
        self.vid = cv2.VideoCapture(video_source)
        if not self.vid.isOpened():
...
class CameraApp:
    def __init__(self, window, window_title, video_source=0):
        ...
        # Adjust the canvas size to 9/10 of the window width and height
        self.width = int(window_width * 0.9)  # 90% of window width
        self.height = int(window_height * 0.9)  # 90% of window height

        # Set video capture resolution to the desired size
    ...
    def update(self):
        ret, frame = self.vid.get_frame()
        if ret:
            # Resize the frame to fit the canvas size (if not already resized)
