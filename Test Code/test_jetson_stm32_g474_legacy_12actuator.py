"""Versi LEGACY dari test_jetson_stm32_g474.py - down-frame 24 byte, 12 actuator
(termasuk arm/RArm/LArm, index 8-11), dari sebelum keputusan "arm gajadi" yang
motong actuator jadi cuma 8.

⚠️ PENTING: script ini TIDAK nyambung ke firmware STM32 SEKARANG
(Code/NucleoG474RE/Core/Src/main.c) - firmware itu udah expect down-frame 20
byte (8 actuator), bukan 24 byte lagi. Kirim frame 24 byte ke firmware
sekarang = ukuran gak match, DIBUANG otomatis sama
`HAL_UARTEx_ReceiveToIdle_IT` (bukan error, cuma gak pernah dibales - keliatan
TIMEOUT terus).

⚠️ Bagian relay GCS di up-frame (`FORMAT_GCS_RELAY`) juga masih format LAMA
(16 byte, masih ada `mode`+`armWidenNarrow`), beda dari firmware sekarang yang
udah 14 byte (`mode`+`armWidenNarrow` dihapus, lihat ROS2_BRIEF.md). Sengaja
dibiarin lama, asumsi node ROS2 yang ditest pakai file ini belum migrasi ke
dua-duanya (bukan cuma arm doang). Kalau ternyata node-nya udah migrasi ke
format GCS baru tapi belum ke actuator baru, kabarin - file ini perlu dipecah
jadi 2 varian terpisah.

⚠️ `pantiltArah` di down-frame LAMA ini juga masih 1 field enum (0-4, cuma 1
arah), beda dari firmware sekarang yang udah pecah jadi `pantiltHorizontal` +
`pantiltVertical` terpisah (bisa gerak diagonal). Alasannya sama - dibiarin
lama, asumsi node yang ditest belum migrasi ke ini juga.

Kegunaan: ngetes/verifikasi node ROS2 (STM32 Interface Node) yang masih pakai
struct/format lama (24 byte, 12 actuator). Kalau node-nya udah di-update ke
20 byte/8 actuator, pakai `test_jetson_stm32_g474.py` (versi baru), bukan file
ini.
...
Up-frame (balasan) di file ini 20 byte (16 byte relay GCS lama + 4 byte tail)
- beda dari firmware sekarang yang up-frame-nya 18 byte (14 byte relay GCS
baru + 4 byte tail, lihat catatan di atas). Jumlah actuator gak ngefek ke
up-frame, tapi ukuran totalnya tetap beda karena perubahan format GCS.
