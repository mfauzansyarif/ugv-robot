...
1. LRF butuh POWER SENDIRI, terpisah dari sinyal data:
   ...
   Kalau pin ini belum tersambung, LRF gak akan pernah respons apapun,
   gak peduli protokol/baudrate kita benar atau salah.
...
2. LEVEL TEGANGAN SINYAL - LRF itu device 3.3V (bukan 5V!).
   ...
   sinyal 5V sebagai logic HIGH valid (beda dari kamera yang didesain buat
   CMOS 5V). CEK VCC yang disuplai ke modul HW-CS1G - idealnya 3.3V kalau
   mau nyambung ke LRF.
...
kirim 1 frame command, LRF balas 1 frame respons. Checksum semua command
SAMA: (jumlah semua byte sebelumnya) XOR 0x50.
...
Contoh command "baca jarak" (CCh, Quick SMM 1):
  ...
  command byte yang tadi dikirim. Jarak disimpan IEEE754 float32 little-endian.

Sebelum power-on, LRF otomatis kirim string identifikasi ("LRF127 x.x.x")
~50ms setelah power tersambung - TANPA diminta. Dipakai sebagai diagnostik
paling dasar (lrf_dengarkan_pasif / scan_semua_baudrate_pasif) karena gak
bergantung protokol/checksum kita benar atau salah sama sekali.
