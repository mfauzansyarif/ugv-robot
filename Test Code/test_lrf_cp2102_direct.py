...
TANPA modul RS485 (HW-CS1G) sama sekali. Langkah isolasi paling bersih
setelah kemungkinan di jalur RS485 (voltage mismatch modul, TX/RX modul,
kontinuitas A/B, modul rusak) belum ketemu penyebabnya - sekarang cuma
CP2102 <-> LRF langsung, gak ada perantara.

====================
WIRING - PENTING
====================
1. CP2102 breakout board KEBANYAKAN punya jumper/switch pilihan VCC 3.3V
   atau 5V (label "3V3/5V"). LRF itu device 3.3V ("Serial interface is
   UART (3.3V)" - datasheet resmi) - SET JUMPER KE 3.3V sebelum nyambung
   ke LRF (ini yang mungkin jadi masalah di modul RS485 sebelumnya kalau
   modul itu VCC 5V).
...
3. LRF butuh POWER TERPISAH dari sinyal data:
     pin 4, 5 = supply voltage 3.3-5.4V (boleh dari CP2102 kalau board-nya
                nyediain pin VCC output, atau sumber lain)
     pin 6, 7 = GND - WAJIB disambung ke GND CP2102 juga (referensi
                bersama, biar level sinyal RXD/TXD terbaca benar)
     pin 3    = POWER ON/SHUTDOWN - boleh floating (ada pull-up internal
                510kOhm, floating = otomatis POWER ON, bukan shutdown -
                sudah dikonfirmasi bukan penyebab masalah)

====================
KONSEP KOMUNIKASI (dikonfirmasi dari kode Jetson yang pernah kerja +
datasheet resmi - TIDAK diubah dari versi sebelumnya)
====================
UART murni, default baudrate pabrik 115200 (unit ini kemungkinan sudah
pernah direkonfigurasi ke 9600 - makanya tetap ada scan_semua_baudrate_pasif).
8 data bit, no parity, 1 stop bit. Checksum semua command: (jumlah semua
byte sebelumnya) XOR 0x50. Request "baca jarak": kirim
[0xCC,0x10,0x00,0x00]+checksum, balasan 22 byte diawali 0x59 0xCC, jarak
di byte[2:6] sebagai float32 little-endian. LRF otomatis kirim string
identifikasi ("LRF127 x.x.x") ~50ms setelah power-on TANPA diminta - dasar
dari lrf_dengarkan_pasif().
...
# device-nya sendiri - koreksi dilakukan di software (pola sama kayak
# kalibrasi sudut pantilt M_VERT1/B_VERT dkk).
#
# Model linear: jarak_terkoreksi = jarak_mentah * SKALA_KOREKSI + OFFSET_KOREKSI_METER
#
# CARA MENENTUKAN NILAINYA (WAJIB empiris, jangan asal tebak):
#   1. Ukur ke 2-3 target dengan jarak UDAH PASTI diketahui (meteran/laser
#      distance meter terpercaya), beda-beda jauh (misal 5m, 15m, 30m).
#   2. Bandingkan hasil LRF vs jarak asli di tiap titik.
#   3. Selisih SAMA di semua jarak (misal selalu -0.5m) -> OFFSET_KOREKSI_METER
#      = +0.5, SKALA_KOREKSI tetap 1.0.
#   4. Selisih BERUBAH sebanding jarak (misal -2% di semua jarak) -> SKALA_KOREKSI
#      yang disesuaikan (contoh: LRF baca 98% dari jarak asli -> SKALA_KOREKSI
#      = 1/0.98 = 1.0204).
#   5. Kombinasi keduanya -> hitung regresi linear sederhana dari beberapa
#      titik data (jarak_asli vs jarak_LRF), sama seperti cara
#      M_VERT1/B_VERT dihitung dulu.
SKALA_KOREKSI = 1.0
