...
ASCII, 1 baris per update, newline-terminated:
  ...
  - "M"              = tag frame ini "Motor AC". Nanti kalau nambah 12 motor
                        linear, dikasih tag BEDA, misal "L <speed1> ...
                        <speed12>\\n" di baris terpisah, biar frame motor AC
                        gak ikut kepanjangan. STM32 cek karakter pertama tiap
                        baris (strtok token ke-1) buat nentuin jenis frame.
  - speed1..speed4   = integer -100..100 buat masing2 dari 4 motor AC.
                        Tanda (+/-) nentuin arah (+ = maju/CW, - = mundur/
                        CCW), angka nentuin besaran speed - di-mapping
                        firmware ke frekuensi pulsa PULS masing2 motor
                        (lihat diskusi position-control-as-continuous-motion
                        di test_position_control_arduino.ino). 0 = motor stop.

Kenapa ASCII (bukan binary/checksum kaya protokol pantilt-LRF): gampang
di-debug manual (bisa diketik langsung di PuTTY/Arduino Serial Monitor tanpa
Python), parsing STM32 gampang pakai strtok() (pola sama kayak
main_code_ros.cpp versi lama), dan ukurannya (4-16 angka) jauh dari bikin
delay - di 115200 baud 1 baris ~20-30 karakter cuma ~2-3ms buat kekirim,
sedangkan frame dikirim tiap 50ms (20Hz).

Kenapa frame dikirim TERUS-MENERUS oleh background thread (bukan cuma sekali
tiap ganti command): kalau 1 baris hilang/kepotong (serial noise), baris
berikutnya 50ms kemudian otomatis "membetulkan" - motor gak nyangkut lama di
state salah. Ini juga buka jalan buat firmware STM32 pasang WATCHDOG: gak
ada baris valid masuk >~300ms -> otomatis stop semua motor, nutup celah
mirip bug lama "default_message 2 kata" di ros_jetson.py (field kurang ->
parsing crash MCU).

PENTING buat firmware STM32 (di luar cakupan file Python ini, WAJIB
diterapkan biar gak mengulang bug lama): sebelum manggil atoi()/atof() ke
hasil strtok(), WAJIB cek hasilnya bukan NULL, dan jumlah token harus PAS 5
(tag "M" + 4 angka). Baris gak lengkap/rusak harus DIABAIKAN total (bukan
diproses sebagian dengan default 0 diam-diam) - biar watchdog timeout yang
ambil alih (stop motor), bukan parsing crash kayak main_code_ros.cpp lama.
