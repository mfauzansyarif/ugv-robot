...
====================
Kamera (Sony FCB-EV7520) protokol native chip-nya VISCA (datasheet resmi),
TAPI modul joystick+RS485 yang nempel di kamera terima Pelco-D di sisi
RS485-nya dan translate ke VISCA secara internal - jadi file ini TETAP
kirim Pelco-D ke modul itu. CONFIRMED jalan hasil scan manual: baudrate
9600, address 1.

LRF (Noptel LRF127) SEKARANG GAK LANGSUNG DI BUS RS485 LAGI - byte mentah
respons LRF (bisa ngandung 0xFF di tengah data float-nya) beresiko nabrak
protokol Pelco-D pantilt/kamera di bus yang sama. Solusinya: LRF sekarang
di belakang BRIDGE STM32 (firmware_snapshot_lrf_bridge_g431kb.c, jalan di
NUCLEO-G431KB) - bridge ini nempel di bus RS485 bersama sebagai device
address=2, terima command Pelco-D-style, translate ke protokol native LRF
lewat TTL langsung (USART2 STM32, tanpa modul RS485), lalu BUNGKUS ULANG
hasilnya jadi frame Pelco-D 7-byte sebelum dikirim balik ke bus. Byte
mentah LRF gak pernah nongol di bus bersama.

Pantilt tetap pakai protokol custom hasil reverse-engineer sebelumnya
(lihat test_rs485_protokol_awal.py) - device ini gak ada datasheet resminya.

====================
SOAL BAUDRATE - WAJIB DISAMAKAN SEBELUM DIPASANG KE 1 BUS BERSAMA
====================
RS485 itu cuma sinyal listrik - semua device di 1 bus HARUS di-set decode
di baudrate yang SAMA, gak bisa beda-beda.
  - Pantilt        : 9600 (hasil reverse-engineer, gak ada cara ganti)
  - Kamera         : CONFIRMED 9600 (hasil scan manual)
  - Bridge STM32   : 9600 di USART1 (di-hardcode di
                      firmware_snapshot_lrf_bridge_g431kb.c,
...
# ==================== PANTILT (custom, dari test_rs485_protokol_awal.py) ====================
...
# LRF gak langsung di bus RS485 lagi - nyambung TTL langsung ke USART2 STM32
# (firmware_snapshot_lrf_bridge_g431kb.c). Bridge STM32 nempel di bus bersama
# sebagai address=2, terima command Pelco-D-style di bawah ini, translate ke
# protokol native LRF, baca hasilnya, lalu BUNGKUS ULANG jadi frame Pelco-D
# 7-byte sebelum kirim balik ke bus - byte mentah LRF gak pernah nongol di
# bus bersama. CONFIRMED jalan end-to-end (tes pakai emulasi LRF palsu,
# hasilnya cocok).
#
...
# custom bikinan sendiri (bridge ini BUKAN Pelco-D asli, cuma KEBETULAN
# pakai bungkus frame yang sama biar seragam di bus).
...
def bridge_lrf_baca_respons(ser, label=""):
    ...
    firmware_snapshot_lrf_bridge_g431kb.c) - jadi respons kosong di sini itu
    NORMAL, bukan berarti bus/wiring rusak. Coba ulang aja."""
    respons = baca_frame_7byte(ser)
    if respons is None:
