...
/**
  ...
  *                    Catatan wiring: salah satu pin "PB7" di pinout board
  *                    Nucleo-32 ternyata gak ke-connect fisik - sudah dipindah
  *                    ke PB7 yang benar (USART1 bus sudah tes echo, sehat).
  ******************************************************************************
  * @attention
  *
...
/* USER CODE BEGIN PD */
/* ====================
 * KONSEP BRIDGE INI (diskusi lengkap di chat):
 * ====================
 ...
 * LPUART1 (PA2=TX, PA3=RX) -- DEBUG ONLY lewat ST-LINK VCP, gak pernah
 *   nyentuh USART1/USART2 supaya gak ganggu protokol asli.
 *
 * PENTING: JANGAN print/transmit blocking di dalam HAL_UART_RxCpltCallback
 * (ISR) - bus 9600 baud (~1ms/byte), ISR sibuk >1ms bikin byte KELEWAT atau
 * OVERRUN ERROR (pernah kejadian di versi diagnostik echo, fix ada di
 * HAL_UART_ErrorCallback di bawah). ISR di sini cuma nyusun frame buffer,
 * print & proses berat semua di main loop.
 * ==================== */
 ...
 *        ser.reset_input_buffer() di test_bus_pantilt_kamera_lrf.py.
 */
