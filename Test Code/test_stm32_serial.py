...
CATATAN: "keyboard_teleop" & "stm32_bridge" di bawah ini nama node ROS2 dari
struktur PRA-REFACTOR yang udah gak ada di repo (node aktif sekarang:
core_node.py + stm32_interface_node.py, protokolnya belum tentu sama) -
disebut cuma buat konteks kenapa format command di sini begini.
...
Format command sama persis kaya yang dikirim stm32_bridge (versi lama) - 6 field
dipisah spasi, newline-terminated:
"mode pwm_value mode_fbw_steer mode_mid_elv init_motor dir_motor"

Baudrate default 57600 - sudah kebukti empiris ini yang beneran dipakai firmware,
BUKAN 115200 kaya di komentar main_code_ros.cpp (source lama dari codebase
pre-STM32G474 yang udah gak ada di repo, cuma konteks historis - 115200 bikin
data ke-korup pas dikirim, "rem" jadi kebaca command lain yang gak dimaksud).
...
Gak ada mekanisme ACK dari STM32 di jalur ini (firmware lama cuma nge-print
