...
# Arti motor_individual_id (di-REDEFINE dari GCS) - 8 actuator individual
# (1-8), PLUS 1 mode pasangan (9) buat steer belakang kiri+kanan gerak
# BERLAWANAN bareng (pola sama kayak logic steer normal), dipakai kalau
# operator mau belok cepat pakai tombol individual tanpa nunggu threshold
# joystick x_joy1.
ID_NORMAL = 0
...
class CoreNode(Node):
    def __init__(self):
        ...
        self._health = Health()
        self._health.stm32_status = 1
        self._deteksi = PersonDetection()
    ...
    def _hitung_actuator(self, relay: GcsRelay):
        ...
        if relay.motor_individual_id != ID_NORMAL:
            ...
            elif mid == ID_STEER_BELAKANG_PASANGAN:
                # Kiri & kanan BERLAWANAN, pola sama kayak logic steer
                # normal (bukan gerak sama arah kayak kalibrasi) - biar
